...
const string strMessageMagic = "Veterancoin Signed Message:\n";
...
{
    // Veterancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Payout is halved every year.
    nSubsidy >>= (nHeight / 262800);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60 * 60; // Difficulty target: 1 hour
static const int64 nTargetSpacing = 2 * 60; // New block every 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Veterancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
/* current difficulty formula- thank you megacoin - kimoto gravity well */
...
{
    static const int64	BlocksTargetSpacing	= 2 * 60; // 2 mins
unsigned int	TimeDaySeconds	= 60 * 60 * 24;
int64	PastSecondsMin	= TimeDaySeconds * 0.2;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Veterancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Veterancoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	/*
		2014-02-05 01:15:07 block.nTime = 1317972665

		2014-02-05 01:15:07 block.nNonce = 2084524493

		2014-02-05 01:15:07 block.GetHash = 
	*/
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Veterancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VeterancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VeterancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VeterancoinMiner(CWallet *pwallet)
{
    printf("VeterancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("veterancoin-miner");
    ...
    try { loop {
        ...
        printf("Running VeterancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VeterancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VeterancoinMiner, pwallet));
}
